...
import { RebalanceConfirmModal } from "./modals/RebalanceConfirmModal";
...
export function RebalanceAnalysisCard({
  ...
  const [selectedIntent, setSelectedIntent] = useState<RebalanceIntent | null>(
    null
  );
  ...
  const handleExecuteClick = (intent: RebalanceIntent) => {
        ...
        {analysis.rebalanceIntents.map((intent, index) => (
          <Card key={intent.id}>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-600 font-bold text-lg">
                      {formatUSD(
                        parseFloat(intent.netBenefit.netYearlyGainUSD)
                      )}
                    </span>
                  </div>
                </div>
              ...
              <Button
                className="w-full"
                onClick={() => handleExecuteClick(intent)}
                disabled={!intent.guardrailsStatus.every((g) => g.passed)}
              >
                {intent.guardrailsStatus.every((g) => g.passed)
                  ? "Execute Rebalance"
        ...
        {analysis.rebalanceIntents.length === 0 && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Your positions are already in the best available protocols.
